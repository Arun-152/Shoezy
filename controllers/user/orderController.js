...
const calculateOrderTotals = (order) => {
  ...
  let subtotal = 0;
  let activeItemsCount = 0; const originalItemsCount = order.items.length; order.items.forEach(item => { if (item.status !== 'Returned' && item.status !== 'Cancelled') { subtotal += item.totalPrice; activeItemsCount++; } });
